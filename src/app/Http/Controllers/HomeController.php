...
class HomeController extends Controller
{
    ...
    public function question_edit(Request $request)
    {
        ...
        // dd($choices[0]);
        return view('question_edit', compact('prefecture', 'question', 'choices'));
    }
    public function question_update(Request $request, $question_id)
    {
        $choices = Choice::where('question_id', $question_id)->get();
        $choice_0 = $choices[0];
        $choice_0->name = $request->choice_name[0];
        $choice_0->save();
        $choice_1 = $choices[1];
        $choice_1->name = $request->choice_name[1];
        $choice_1->save();
        $choice_2 = $choices[2];
        $choice_2->name = $request->choice_name[2];
        $choice_2->save();


        // foreach($choices as $index => $choice) {
        //     $choice->name = $choice_name_{$index};
        //     $choice->save();
        // }
        // $choices_name = $request->choice_name;
        // dd($choices_name);
